...
# Let's look at only a single picture and one of the two replications:
...
(SST <- sum( (y - mean(y))^2 ))
################################
## Find the residual variance  after model fit: 
...
(SSE <- sum( (y1 - mean(y1))^2 ) + 
        ...
        sum( (y3 - mean(y3))^2 ))
################################
## Find the  variance explained by the model: 
## Sum of Squares for treatment SS(Tr)

## Just by substracting:
(SSTr <- SST - SSE)
## Or directly by defining formula: 
4 * (mean(y1) - mean(y))^2 + 4 * (mean(y2) - mean(y))^2 + 4 * (mean(y3) - mean(y))^2


################################
## All this could be found by  anova() and lm()
